import { z } from "zod";
import { createPlaylist, getAuthUrl, saveToken, getOAuthClient, } from "../utils/oauth.js";
export function registerPlaylistTool(server) {
    // Tool 1: Get the OAuth authorization URL
    server.tool("get_auth_url", "Get the Google OAuth authorization URL. User must visit this URL to grant YouTube access before creating playlists.", {}, async () => {
        const url = getAuthUrl();
        return {
            content: [
                {
                    type: "text",
                    text: `Please visit this URL to authorize YouTube access:\n\n${url}\n\n` +
    ...
    server.tool("save_auth_code", "Save the OAuth authorization code received after visiting the auth URL", {
        code: z
            .string()
    ...
    server.tool("create_playlist", "Create a YouTube playlist and add videos to it (requires prior OAuth authorization)", {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Playlist "${title}" created successfully!\n\n` +
